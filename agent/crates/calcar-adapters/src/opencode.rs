...
//!   travel in the spawn `argv` (e.g. `opencode run "<prompt>"`) until ConPTY
//!   greens. Returning `Ok` without delivering would lie to core.
//! - `respond_approval` always returns [`AdapterError::Unsupported`]. Headless
//!   `opencode run` exposes no stdin approval-reply protocol over plain pipes;
//!   approval *requests* are still parsed into `ApprovalRequired` events for
//!   the permission manager, which owns expiry, single-use, and
//!   double-accept rejection. This adapter never resolves approvals itself.
...
//!
//! Every line is matched against the `OPENCODE_` namespace only. Lines without
//! the prefix are plain CLI chatter and skipped. A trailing `\r` (from
//! `cmd.exe` `\r\n` fixtures) is stripped before matching. Grammar:
//!
//! ```text
//! OPENCODE_SESSION <id>                  -> snapshot session/resume pointer, no event
//! OPENCODE_START <summary>               -> AgentStarted
//! OPENCODE_STEP <summary>                -> CommandStarted
//! OPENCODE_DONE <summary>                -> CommandCompleted
//! OPENCODE_FILE <path> [note]            -> FileChanged, detail_pointer = <path>
//! OPENCODE_APPROVAL <req-id> <summary>   -> ApprovalRequired, detail_pointer = <req-id>
//! OPENCODE_INPUT <summary>               -> InputRequired
//! OPENCODE_ERROR <summary>               -> ErrorOccurred
//! OPENCODE_EXIT ok <summary>             -> WorkflowCompleted
//! OPENCODE_EXIT fail <summary>           -> ErrorOccurred
//! ```
//!
//! Canonical canned fixture:
//!
//! ```text
//! b"OPENCODE_SESSION sess_01\nOPENCODE_START drafting plan\nOPENCODE_STEP edit src/main.rs\nOPENCODE_FILE src/main.rs updated\nOPENCODE_APPROVAL req_7 run tests\nOPENCODE_INPUT confirm deploy\nOPENCODE_DONE all green\nOPENCODE_EXIT ok finished\n"
//! ```
//!
//! Skipped-never-fatal fixture:
//!
//! ```text
//! b"plain chatter\nOPENCODE_\nOPENCODE_BOGUS x\nOPENCODE_EXIT maybe later\n"
//! ```
//!
//! # Resume
//!
//! `capabilities.resume` is true: opencode supports continuing a session via a
//! `--session <id>` style argv flag (confirm exact flag against the installed
//! binary at merge; the adapter passes `argv` through opaquely). When the CLI
//! prints `OPENCODE_SESSION <id>`, `snapshot().resume_pointer` and
...
/// Line namespace for this provider. Keeps opencode markers disjoint from the
/// claude/codex grammars so one CLI's chatter never parses as another's event.
const PREFIX: &str = "OPENCODE_";

/// Zero-timeout poll used to refresh the cached exit code without blocking.
const EXIT_POLL: Duration = Duration::from_millis(0);

/// opencode backend. One running [`PlainChild`] at most; core sees only
