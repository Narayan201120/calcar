//! Claude Code CLI adapter (PLAN P5).
//!
//! # Failure modes first
//!
...
//!   travel in the spawn `argv` (e.g. `claude -p "<prompt>"`) until ConPTY
//!   greens. Returning `Ok` without delivering would lie to core.
//! - `respond_approval` always returns [`AdapterError::Unsupported`]. Claude
//!   permission prompts need a TTY or pre-approved policy flags; no pipe reply
//!   channel exists in headless `-p` mode. Approval *requests* are still parsed
//!   into `ApprovalRequired` events for the permission manager, which owns
//!   expiry, single-use, and double-accept rejection. This adapter never
//!   resolves approvals itself.
...
//!
//! Every line is matched against the `CLAUDE_` namespace only. Lines without
//! the prefix are plain CLI chatter and skipped. A trailing `\r` (from
//! `cmd.exe` `\r\n` fixtures) is stripped before matching. Grammar:
//!
//! ```text
//! CLAUDE_SESSION <id>                  -> snapshot session/resume pointer, no event
//! CLAUDE_START <summary>               -> AgentStarted
//! CLAUDE_STEP <summary>                -> CommandStarted
//! CLAUDE_DONE <summary>                -> CommandCompleted
//! CLAUDE_FILE <path> [note]            -> FileChanged, detail_pointer = <path>
//! CLAUDE_APPROVAL <req-id> <summary>   -> ApprovalRequired, detail_pointer = <req-id>
//! CLAUDE_INPUT <summary>               -> InputRequired
//! CLAUDE_ERROR <summary>               -> ErrorOccurred
//! CLAUDE_EXIT ok <summary>             -> WorkflowCompleted
//! CLAUDE_EXIT fail <summary>           -> ErrorOccurred
//! ```
//!
//! Canonical canned fixture:
//!
//! ```text
//! b"CLAUDE_SESSION sess_02\nCLAUDE_START reviewing diff\nCLAUDE_STEP run cargo check\nCLAUDE_FILE src/lib.rs reviewed\nCLAUDE_APPROVAL req_9 edit Cargo.toml\nCLAUDE_INPUT pick a branch\nCLAUDE_DONE review posted\nCLAUDE_EXIT ok finished\n"
//! ```
//!
//! Skipped-never-fatal fixture:
//!
//! ```text
//! b"plain chatter\nCLAUDE_\nCLAUDE_BOGUS x\nCLAUDE_EXIT maybe later\n"
//! ```
//!
//! # Resume
//!
//! `capabilities.resume` is true: claude supports resuming a session via a
//! `--resume <id>` style argv flag (confirm exact flag against the installed
//! binary at merge; the adapter passes `argv` through opaquely). When the CLI
//! prints `CLAUDE_SESSION <id>`, `snapshot().resume_pointer` and
...
/// Line namespace for this provider. Keeps claude markers disjoint from the
/// opencode/codex grammars so one CLI's chatter never parses as another's event.
const PREFIX: &str = "CLAUDE_";

/// Zero-timeout poll used to refresh the cached exit code without blocking.
const EXIT_POLL: Duration = Duration::from_millis(0);

/// Claude Code backend. One running [`PlainChild`] at most; core sees only
